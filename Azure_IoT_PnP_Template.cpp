...
#define TELEMETRY_PROP_NAME_LDR "Illuminance"
#define TELEMETRY_PROP_NAME_DISTANCE "Distance"
...
#define PIEZOELECTRIC_PIN 5  // Pin untuk potensiometer

// Definisikan pin untuk potensiometer
#define POTENTIOMETER_PIN 4  // Pin untuk potensiometer

//definisi untuk pin buzzer
#define buzzer 6

// Fungsi untuk membaca nilai dari sensor piezoelektrik
static int get_piezoelectric_value() {
  return analogRead(PIEZOELECTRIC_PIN);  // Membaca nilai analog (0-4096)
}

// Fungsi untuk membaca nilai dari potensiometer
static int get_potentiometer_value() {
  return analogRead(POTENTIOMETER_PIN);  // Membaca nilai analog (0-4096)
}

// Fungsi untuk membuat payload telemetry JSON
static int generate_telemetry_payload(
    uint8_t* payload_buffer,
    size_t payload_buffer_size,
    size_t* payload_buffer_length) {
  ...
  // Membaca data dari sensor ultrasonik dan potensiometer
  float piezoelectric_value = get_piezoelectric_value();
  int potentiometer_value = get_potentiometer_value();
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed starting telemetry JSON object.");

  // Tambahkan data ultrasonik
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("piezoelectric_value"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding ultrasonic_distance property name.");
  rc = az_json_writer_append_double(&jw, piezoelectric_value,2);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding ultrasonic_distance value.");

  // Tambahkan data potensiometer
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("potentiometer_value"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding potentiometer_value property name.");
  rc = az_json_writer_append_int32(&jw, potentiometer_value);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding potentiometer_value value.");

  rc = az_json_writer_append_end_object(&jw);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed closing telemetry JSON object.");
  ...
  payload_buffer[az_span_size(payload_buffer_span)] = '\0'; // Tambahkan null terminator
